...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log('Organization Media Post Data:', JSON.stringify(postData, null, 2));
    ...
    return NextResponse.json({
      success: true,
      message: 'LinkedIn organization media post created successfully',
      data: response.data,
      postType: imageUrl && linkUrl ? 'image+link' : imageUrl ? 'image' : linkUrl ? 'link' : 'text',
    });
  } catch (error: any) {
    console.error('Organization media post error:', error.response?.data || error.message);
    return NextResponse.json(
      {
        error: 'Failed to create LinkedIn organization media post',
